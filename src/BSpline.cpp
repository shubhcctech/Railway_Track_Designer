...
std::vector<Geometry::Point3D> Feature::BSpline::evaluate2D(std::vector<Geometry::Point3D> control_points, float t) {
	int num_control_points = control_points.size();
	std::vector<double> knots = generateUniformKnots(num_control_points);
...
void Feature::BSpline::drawBsplineCurve1D(std::vector<Geometry::Point3D>& control_points, std::vector<GLdouble>& inVertices, std::vector<GLdouble>& inColors)
{
	std::vector<Geometry::Point3D> curve_points = evaluate1D(control_points, 100);
...
void Feature::BSpline::drawBsplineCurve2D(std::vector<Geometry::Point3D>& control_points, std::vector<GLdouble>& inVertices, std::vector<GLdouble>& inColors)
{
	std::vector<Geometry::Point3D> curve_points = evaluate2D(control_points, 100);
	for (Geometry::Point3D point : curve_points)
	{
		inVertices.push_back(point.x());
		inVertices.push_back(point.y());
